...
export class RolesGuard implements CanActivate {
  ...
  async canActivate(executionContext: ExecutionContext): Promise<boolean> {
    const requiredRoles = this.reflector.get<USER_ROLE[]>(ROLES_KEY, executionContext.getHandler());
    if (!requiredRoles) {
      return true;
    }
    ...
    const result = requiredRoles.some(role => userSession.role?.includes(role));
